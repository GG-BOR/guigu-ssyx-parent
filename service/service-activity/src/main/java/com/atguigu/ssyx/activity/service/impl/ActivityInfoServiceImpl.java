...
public class ActivityInfoServiceImpl extends ServiceImpl<ActivityInfoMapper, ActivityInfo> implements ActivityInfoService {
    ...
    public void saveActivityRule(ActivityRuleVo activityRuleVo) {
        ...
        for (ActivitySku activitySku : activitySkuList) {
            activitySku.setActivityId(activityId);//活动id
    ...
    public List<SkuInfo> findSkuInfoByKeyword(String keyword) {
        ...
        //(1)service-product 模块创建接口 据关键字查询sku内容列表
...
//        }
        //②stream写法
        List<SkuInfo> findSkuList2 = skuInfoList.stream().filter(item -> !existSkuIdList.contains(item.getId())).collect(Collectors.toList());
        return findSkuList2;
    }
    ...
    public List<CartInfoVo> findCartActivityList(List<CartInfo> cartInfoList) {
        ...
        Map<Long, Set<Long>> activityIdToSkuIdListMap =
                activitySkuList.stream().collect(Collectors.groupingBy(ActivitySku::getActivityId,
        ...
        if(!CollectionUtils.isEmpty(activityIdToSkuIdListMap)){
            //遍历activityIdToSkuIdListMap集合,用了迭代器iterator
            ...
            while (iterator.hasNext()){
                ...
                ActivityType activityType = currentActivityRuleList.get(0).getActivityType();//获取活动类型
                //判断活动类型:满减和打折
